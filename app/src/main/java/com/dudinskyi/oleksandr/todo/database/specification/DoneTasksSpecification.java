...
public class DoneTasksSpecification implements Specification<RealmTaskData> {

    @Override
    public RealmResults<RealmTaskData> toRealmResults(Realm realm) {
        return realm.where(RealmTaskData.class).
                equalTo(RealmTaskData.stateField, 1).findAll();
    }
    ...
    public RealmTaskData toRealmResult(Realm realm) {
        return realm.where(RealmTaskData.class).
                equalTo(RealmTaskData.stateField, 1).findFirst();
    }
